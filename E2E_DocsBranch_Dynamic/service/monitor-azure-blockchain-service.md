...
Azure Blockchain Service creates monitoring data using Azure Monitor, which is a full stack monitoring service in Azure that provides a complete set of features to monitor your Azure resources.
 
...
Azure Blockchain Service collects the same kind of monitoring data as other Azure resources, which are described. See [Monitor Azure Blockchain Service data reference](#monitor-azure-blockchain-service-data-reference) for a detailed reference of the logs and metrics created by Azure Blockchain Service.
...
Platform metrics and the Activity log are collected automatically, but you must create a diagnostic setting to collect resource logs or forward them outside of Azure Monitor.for the detailed process for creating a diagnostic setting using the Azure portal, CLI, or PowerShell.
...
You can analyze metrics for Azure Blockchain Service with Metrics explorer, navigate to Metrics tab under Monitoring section in ABS resource blade. for details on using the tool. The complete metrics for Azure Blockchain Service are in the namespace Azure Blockchain Service standard metrics.
...
Here are some queries that you can enter in the Log search bar to help you monitor your Azure Blockchain Service members.
...
All resource logs share a top-level common schema with few unique properties specific to the blockchain service. You can refer to the article, details of the Azure Blockchain Service specific properties are covered below  
...
For a list of all Azure Monitor supported metrics (including Azure Blockchain Service).
